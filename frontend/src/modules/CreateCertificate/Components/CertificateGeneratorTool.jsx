...
import "./CertificateGeneratorTool.css";
...
function CertificateGeneratorTool() {
    const {certReceiverName} = useSelector(state => state.certificateIssuer);
    const userData = JSON.parse(localStorage.getItem("user"));
    console.log("certReceiverName :",certReceiverName);
    const [pop, setpop] = useState(false);
    const [name, setname] = useState(certReceiverName);
    const [heading, setheading] = useState('');
    const [desc, setdesc] = useState('');
    const [author, setauthor] = useState(userData.fullname);
    ...
    return (
        <div className="main" style={{background: "white",border:"2px solid black"}}>
            <Popup trigger={pop} setpop={setpop} >
            </Popup>
            <Title style={{textAlign:"center"}}>Certificate Generation Tool</Title>
            <div className="maincontainer">
                <div className="leftmost">
                    <h1 style={{color: "rgb(28, 39, 43)"}}>Templates</h1>
                ...
                <div className="right">
                    <div className="form">
                        <div className="input-box">
                            <span className="details" style={{"white":"rgb(28, 39, 43)"}}>Heading</span>
                            <input type="text" placeholder="Enter heading" onChange={e => { setheading(e.target.value) }} />
                        </div>
                        <div className="input-box">
                            <span className="details" style={{"white":"rgb(28, 39, 43)"}}>Particpant Name</span>
                            <input type="text" defaultValue={name} placeholder="Enter participant Name" onChange={e => { setname(e.target.value) }} />
                        </div>
                        <div className="input-box">
                            <span className="details" style={{"white":"rgb(28, 39, 43)"}}>Description</span>
                            <textarea type="text" placeholder="Enter Description" onChange={e => setdesc(e.target.value)} />
                        </div>
                        <div className="input-box">
                            <span className="details" style={{"white":"rgb(28, 39, 43)"}}>Author Name</span>
                            <input type="text" defaultValue={author} placeholder="Enter Author Name" onChange={e => setauthor(e.target.value)} />
                        </div>
                        <div className="input-box">
                            <span className="details" style={{"white":"rgb(28, 39, 43)"}}>Logo URL</span>
                            <input type="text" placeholder="Enter logo URL" onChange={e => setlogo(e.target.value)} />
                        </div>
                        <Button size="large" type="primary" onClick={showModal}>Review and Confirm</Button>
                        {isModalOpen && <ConfirmDetailsModal isModalOpen={isModalOpen} handleCancel={handleCancel} componentRef={componentRef} />}
                    </div>
                </div>
            </div>
        </div>
